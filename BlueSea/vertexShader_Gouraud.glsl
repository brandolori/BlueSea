...
void main()
{
    ...
    if (sceltaS==3)  // per onde
    {
        ...
        float offset = sin(time + 100.0 * aPos.x);
        v.y	= v.y + offset;
	    ...
	    V = -eyePosition.xyz;
	    L = (eyeLightPos - eyePosition).xyz;
        vec3 normal = normalize(vec3(cos(time + 100.0 * aPos.x), sin(time + 100.0 * aPos.x), 0));
	    N = mat3(View) * transpose(inverse(mat3( Model))) * normal;
    }
